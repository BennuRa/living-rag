...
def test_service_maps_full_trace_details() -> None:
    ...
    assert '"intent": "refund_request"' in (
        replay.nodes[0].output_summary or ""
    )

    assert len(replay.tool_calls) == 1
    assert replay.tool_calls[0].tool_name == "get_order"
    assert replay.tool_calls[0].latency_ms == 8.0
    assert '"order_no": "O2025001"' in (
        replay.tool_calls[0].input_summary or ""
    )
    ...
    assert '"approval_task_id": "approval-001"' in (
        replay.audit_logs[0].detail or ""
    )
